...
{
    xPos = p * qSin(qDegreesToRadians(ze)) * qSin(qDegreesToRadians(az));
    yPos = p * qSin(qDegreesToRadians(ze)) * qCos(qDegreesToRadians(az));
...
Point Point::transform(double *a)
{
    double x, y, z;
    if(zPos == 0)
    {
...
Point Point::multArray(const Point& p1, const Point& p2)
{
    return Point(p1.xPos * p2.xPos, p1.yPos * p2.yPos, p1.zPos * p2.zPos);
}
